...
from forbiddenfruit_0_1_4 import curse  # type: ignore
import yml_6_0 as yml  # type: ignore
    ...
    if not name:
        if isinstance(data, dict):
            ...
            if 'name' not in vars(data).keys():
                raise NameExpectedError(NameExpectedError.text(data))
            name = data.name
    extension = name.split('.')[-1]  # type: ignore  # TODO: fix
    match extension:
        case 'pickle':
